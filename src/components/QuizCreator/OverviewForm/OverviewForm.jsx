    ...
    return (
        <div className='fixed w-120 py-6 px-7 rounded-lg bg-white'>
            <div className='flex justify-between items-center'>
                <h2 className='m-0 text-left text-2xl text-pink-hot 
                            font-quicksand font-bold'>
                    Create a quiz
                </h2>
                <button className='border-none rounded-full w-8 h-8 
            ...
            <div className='mt-5 mb-2'>
                <form>
                    <div className='form-element'>
                        ...
                        <input className='py-3 px-4 border-none rounded-lg bg-gray 
                    ...
                    <div className='form-element'>
                        ...
                        <div className='flex flex-row flex-wrap text-sm font-quicksand'>
                            {subjects.map((x, i) =>
                                <div key={x.id}>
                                    <input className='hidden selected-subject' type='checkbox' id={x.id} name='subject'
                                        ref={r => subjectsRef.current[i] = r}
                                        onChange={() => handleSelectSubject(x.id)} />
                                    <label className='flex m-1 py-1 px-3 rounded-3xl bg-gray 
                            ...
                            <span className='flex items-center ml-4 text-pink-hot font-bold 
                        ...
                        <div className='flex justify-between py-1 px-4 font-quicksand font-semibold'>
                            {levels.map(x =>
                                <div className='flex flex-col items-center hover:opacity-80' 
                                    key={x.name}>
                                    <input className='hidden selected-level' type='radio' id={x.name} name='level' 
                                    ...
                                    <label className='mt-2 text-sm text-gray-dark cursor-pointer' 
                                        htmlFor={x.name}>
                                        {x.name}
                                    </label>
                                </div>
                    ...
                    <div className='flex justify-end mt-6'>
                            <FancyButton custom='mr-2' type='cancel'>Cancel</FancyButton>
